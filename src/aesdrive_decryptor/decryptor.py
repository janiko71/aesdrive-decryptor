"""Module principal de déchiffrement AES Drive."""
...
from .constants import (
    DEFAULT_FILE, KDF_ITERATIONS, DEFAULT_PWD, PWD_ENCODING,
    HEADER_LENGTH, SECTOR_LENGTH, SUPPORTED_EXTENSIONS
)
from .security import SecureMemory, secure_zero_memory

# Import du module res (à adapter selon la nouvelle structure)
try:
    from res import DataFile, print_parameter, TERM_RED, TERM_BOLD, TERM_RESET
except ImportError:
    # Fallback si le module res n'est pas disponible
    print("⚠️ Module res non trouvé. Certaines fonctionnalités peuvent être limitées.")
    DataFile = None
    print_parameter = lambda k, v: print(f"{k}: {v}")
    TERM_RED = TERM_BOLD = TERM_RESET = ""
...
class AESDecryptor:
    ...
    def load_encrypted_file(self, filepath: str) -> Tuple[object, os.stat_result, object]:
        ...
        try:
            ...
            if DataFile:
                ...
                # Fallback simple si DataFile n'est pas disponible
                data_file = type('DataFile', (), {'file_header': file_header})()
    ...
    def derive_keys(self, password: str, data_file: object) -> Tuple[bytes, bytes]:
    ...
    def decrypt_header(self, header_key: bytes, iv: bytes, data_file: object) -> bytes:
    ...
    def decrypt_file(self, filepath: str, password: str) -> None:
        ...
        try:
            ...
            try:
                ...
                # Afficher les informations du fichier de données si disponible
                if hasattr(data_file, '__dict__') and hasattr(data_file, 'file_salt'):
                    try:
                        from res import print_data_file_info
                        print_data_file_info(data_file)
                    except ImportError:
                        print_parameter("Fichier de données", "Chargé")
